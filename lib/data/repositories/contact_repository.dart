  ...
  Future<List<Map<String, dynamic>>> getRegisteredContacts() async {
    try {
      ...
      final phoneNumbers = contacts.where((contact) => contact.phones.isNotEmpty).map((contact){
        return {'name':contact.displayName,'phoneNumber':contact.phones.first.number.replaceAll(RegExp(r'[^\d+]'), ''),'photo':contact.photo}
      }).toList();
      ...
      final matchedContacts = phoneNumbers.where((contact){
        ...
        return registeredUsers.any((user) => user.phoneNumber==phoneNumber && currentUserId != user.uid);
      }).map((contact){
